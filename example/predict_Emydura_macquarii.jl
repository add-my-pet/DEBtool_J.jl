...
function predict(par, data, auxData)# predict
    ...
    (; temp, tLt) = auxData
    (; Lb, Li, Lim, Lp, Lpm, Ri, Wwb, Wwi, Wwim, Wwp, Wwpm, ab, ab30, am, psd, tp, tpm) = data

    cPar = parscomp_st(par)
    (; g, k, l_T, v_Hb, v_Hp, L_m, w, k_M, w, L_T, U_Hb, U_Hp, w_E, w_V, y_E_V, v_Hpm) = cPar
    ...
    d_V = 1.0Unitful.u"g/cm^3"               # cm, physical length at birth at f

    pars_tp = (; g=cPar.g, k, l_T, v_Hb, v_Hp)
    ...
    # time-length
    rT_B = TC * k_M / 3 / (1 + f / g)
    ELw = Lw_i .- (Lw_i - Lw_b) .* exp.(-rT_B .* tLt)
