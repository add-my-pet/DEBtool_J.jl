...
"""
@kwdef struct LifeStages{LS<:Tuple} <: AbstractLifeStages
    lifestages::LS
end
LifeStages(args::Pair...) = LifeStages(args)

Base.values(ls::LifeStages) = ls.lifestages
Base.length(ls::LifeStages) = Base.length(Base.values(ls))
...
# stf
struct StandardFoetalDevelopment <: AbstractStandardFoetalMode end
# stx
struct StandardFoetalDevelopmentX <: AbstractStandardFoetalMode end
# ssj
...
std() = Standard()
stf() = StandardFoetalDevelopment()
stx() = StandardFoetalDevelopmentX()
...
asj() = Accelerated(Isomorph(), Birth(), Puberty(), Isomorph())
# TODO clarify these
...
"""
@kwdef struct DEBOrganism{M<:Mode,LS<:AbstractLifeStages,TR<:AbstractTemperatureResponse,R<:AbstractReproduction}
    mode::M
    lifestages::LS
...
std_organism(;
    ...
    kw...
) = DEBOrganism(; lifestages, temperatureresponse, kw..., mode=std())
stf_organism(;
    temperatureresponse = Arrhenius1parTemperatureResponse(),
    lifestages=LifeStages(
        Foetus() => Birth(),
    ...
    kw...
) = DEBOrganism(; lifestages, temperatureresponse, kw..., mode=stf())
stx_organism(;
    temperatureresponse = Arrhenius1parTemperatureResponse(),
    lifestages=LifeStages(
        Foetus() => Birth(),
    ...
    kw...
) = DEBOrganism(; lifestages, temperatureresponse, kw..., mode=stx())
ssj_organism(;
    temperatureresponse = Arrhenius1parTemperatureResponse(),
    lifestages=LifeStages(
        Embryo() => Birth(),
